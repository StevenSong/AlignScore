...
def train(datasets, args):
    ...
    trainer.save_checkpoint(
        os.path.join(args.ckpt_save_path, f"{checkpoint_name}_final.ckpt")
    )
...
if __name__ == "__main__":
    ALL_TRAINING_DATASETS = {
        ...
        "hotpot_qa_distractor": {
            "task_type": "qa",
            "data_path": "hotpot_qa_distractor.json",
        },
        "hotpot_qa_fullwiki": {
            "task_type": "qa",
            "data_path": "hotpot_qa_fullwiki.json",
        },
    ...
    parser.add_argument("--val-check-interval", type=float, default=1.0 / 4)
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default=list(ALL_TRAINING_DATASETS.keys()),
        choices=list(ALL_TRAINING_DATASETS.keys()),
    )
        ...
        name: {
            **ALL_TRAINING_DATASETS[name],
            ...
            "data_path": os.path.join(
                args.data_path, ALL_TRAINING_DATASETS[name]["data_path"]
            ),
        }
